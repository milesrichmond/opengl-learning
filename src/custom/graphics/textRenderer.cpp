...
// Required due to glCurrentContext conflictions
TextRenderer::TextRenderer() { valid = false; }

TextRenderer::TextRenderer(const char *vertexPath, const char *fragmentSource,
                           const char *fontSource, int SCR_WIDTH,
                           int SCR_HEIGHT)
    : shader(Shader(vertexPath, fragmentSource)) {
  ...
  // find path to font
  std::string font_name = fontSource;
  if (font_name.empty()) {
...
void TextRenderer::resize(int SCR_WIDTH, int SCR_HEIGHT) {
  glm::mat4 projection = glm::ortho(0.0f, static_cast<float>(SCR_WIDTH), 0.0f,
                                    static_cast<float>(SCR_HEIGHT));
  shader.use();
  glUniformMatrix4fv(glGetUniformLocation(shader.ID, "projection"), 1, GL_FALSE,
